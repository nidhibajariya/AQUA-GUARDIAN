...
# Try local satdata_client inside aqua_guardian first; fallback to repo root satdata_client
sat_local = os.path.join(os.path.dirname(__file__), "satdata_client", "satdata_client")
sat_root = os.path.join(os.path.dirname(os.path.dirname(__file__)), "satdata_client", "satdata_client")
for p in [sat_local, sat_root]:
...
def decide(user_pred, sat_pred, threshold: float) -> str:
...
def main():
    ...
    parser.add_argument("--model", default=default_model, help="Path to model.h5")
